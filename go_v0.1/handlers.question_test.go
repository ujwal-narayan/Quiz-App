...
func TestArticleUnauthenticated(t *testing.T) {
	...
	r.GET("/question/view/:question_id", getQuestion)

	// Create a request to send to the above route
	req, _ := http.NewRequest("GET", "/question/view/1", nil)

	testHTTPResponse(t, r, req, func(w *httptest.ResponseRecorder) bool {
		// Test that the http status code is 200
		statusOK := w.Code == http.StatusOK

		// Test that the page title is "Question 1"
		// You can carry out a lot more detailed tests using libraries that can
		// parse and process HTML pages
		p, err := ioutil.ReadAll(w.Body)
		pageOK := err == nil && strings.Index(string(p), "<title>Question 1</title>") > 0
...
func TestArticleListJSON(t *testing.T) {
	...
	testHTTPResponse(t, r, req, func(w *httptest.ResponseRecorder) bool {
		...
		// an array of Question structs
		p, err := ioutil.ReadAll(w.Body)
		if err != nil {
			return false
		}
		var questions []question
		err = json.Unmarshal(p, &questions)

		return err == nil && len(questions) >= 2 && statusOK
	})
}
...
func TestArticleXML(t *testing.T) {
	...
	testHTTPResponse(t, r, req, func(w *httptest.ResponseRecorder) bool {
		...
		return err == nil && a.ID == 1 && len(a.Question) >= 0 && statusOK
	})
}
